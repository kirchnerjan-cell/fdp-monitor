...
- **Meta verlangt für oEmbed-Metadaten inzwischen meist einen Access Token.** Ohne Token liefert `graph.facebook.com/.../instagram_oembed` nur noch einen generischen Platzhalter-Embed zurück – ohne Account-Name, Vorschaubild oder Bildunterschrift. `update.py` nimmt den Post dann trotzdem auf, warnt aber in der Konsole mit „oEmbed-Antwort ohne Account-Name/Vorschaubild". Betroffene Karten zeigen „Unbekannt" und „Kein Vorschaubild" auf der Website.
  - **Beheben:** Auf [developers.facebook.com](https://developers.facebook.com/apps) kostenlos eine App anlegen (Typ „Sonstige" reicht) → App-Einstellungen → Basis: **App-ID** kopieren → Erweitert: **Client-Token** kopieren. Token-Format: `<App-ID>|<Client-Token>`.
  - Als Umgebungsvariable setzen: `export IG_OEMBED_TOKEN='<App-ID>|<Client-Token>'` vor `python3 update.py`, oder direkt per Flag: `python3 update.py --access-token '<App-ID>|<Client-Token>'`.
  - **Wichtig:** `update.py` fasst bereits bekannte Post-URLs nie erneut an (siehe oben). Ein Post, der schon ohne Metadaten in `data.json` steht, wird durch einen erneuten Lauf mit Token *nicht* nachträglich vervollständigt – dafür den betroffenen Eintrag aus `data.json` löschen und die URL erneut in `posts.txt` eintragen, dann `update.py` (mit Token) laufen lassen.
